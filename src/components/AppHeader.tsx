...
export function AppHeader({
  ...
  const { pullDistance, isRefreshing, isDragging, isReadyToRelease, touchHandlers } = usePullToRefresh();
  ...
  return (
    <div className="app-header-wrapper" {...touchHandlers}>
      {showIndicator && (
        <div
          className="app-header__ptr-indicator"
          style={{
            height: `${Math.max(pullDistance, isRefreshing ? 50 : 0)}px`,
            opacity: Math.min(pullDistance / 40, 1),
          }}
        >
          <div className="app-header__ptr-content">
            {isRefreshing ? (
              <>
                <svg className="app-header__ptr-spinner" viewBox="0 0 24 24" fill="none" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M12 4v4m0 8v4m-8-8h4m8 0h4m-2.828-5.657l-2.829 2.829m-5.657 5.657l-2.828 2.829m0-11.314l2.828 2.829m5.657 5.657l2.829 2.828" />
                </svg>
                <span>画面を更新中…</span>
              </>
            ) : isReadyToRelease ? (
              <>
                <svg className="app-header__ptr-arrow is-up" viewBox="0 0 24 24" fill="none" stroke="currentColor">
              ...
              <>
                <svg
                  className="app-header__ptr-arrow"
                  viewBox="0 0 24 24"
                  fill="none"
                  stroke="currentColor"
      ...
      >
        ...
        <nav className="app-header__tabs">
          <button type="button" className="app-header__tab" onClick={onOpenList}>
            保護犬一覧
          </button>
          <button type="button" className="app-header__tab" onClick={onOpenGallery}>
            ギャラリー
          </button>
        </nav>
